...
function DarazZoomImage({ src, alt }: { src: string, alt: string }) {
    ...
    return (
       <div className={`w-full flex justify-center items-center rounded-xl md:rounded-3xl py-4 md:py-8 ${liquidGlassClass}`}>
          <div 
            className="relative overflow-hidden cursor-crosshair rounded-lg md:rounded-xl shadow-[0_10px_20px_rgba(0,0,0,0.08)] w-[90%] h-[35vh] md:h-[50vh] flex justify-center bg-white/50"
...
const ptComponents = {
  ...
  block: {
    normal: ({children}: any) => <p className="text-gray-700 text-sm md:text-base leading-relaxed mb-4 font-medium">{children}</p>,
    h1: ({children}: any) => <h1 className="text-xl md:text-3xl font-black text-gray-900 mb-4 mt-6">{children}</h1>,
    h2: ({children}: any) => <h2 className="text-lg md:text-2xl font-black text-gray-800 mb-3 mt-5">{children}</h2>,
  },
  marks: {
        ...
        <div className="w-full relative">
           ...
           <div className="flex flex-col items-center text-center mb-10 md:mb-14 relative z-10">
              ...
              <h2 className="text-3xl md:text-5xl font-black text-black tracking-tight">
                 Featured <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-yellow-500">Projects</span>
              </h2>
           </div>

           {/* Heynesh-style Project Grid */}
           <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 relative z-10">
               {displayProjects.map((proj: any, idx: number) => {
                  ...
                  return (
                     <motion.div 
                       ...
                       key={proj._id} 
                       className="group relative rounded-[2rem] overflow-hidden cursor-pointer aspect-[16/11] md:aspect-[16/10] bg-neutral-900 border border-black/10 shadow-[0_15px_35px_rgba(0,0,0,0.08)] transition-all duration-500 hover:shadow-[0_25px_50px_rgba(249,115,22,0.15)]" 
                     ...
                     >
                        ...
                        {proj.image && (
                           <Image 
                             ...
                             alt={proj.title} 
                             fill
                             className="object-cover object-center transition-transform duration-700 ease-out group-hover:scale-105 opacity-85 group-hover:opacity-100" 
                           />
                        )}
                        
                        {/* Gradient Overlay for Text Visibility */}
                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/35 to-black/20 group-hover:from-black/95 transition-colors duration-500 pointer-events-none" />

                        {/* Heynesh Card Layout Content */}
                        <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-between z-10 pointer-events-none">
                           ...
                           {/* Bottom Row: Title + Tagline + Floating Circle Arrow */}
                           <div className="flex justify-between items-end gap-4 w-full">
                              <div className="flex flex-col max-w-[80%]">
                                 <h3 className="text-xl md:text-3xl font-black text-white group-hover:text-orange-400 transition-colors duration-300 tracking-tight leading-tight">
                                    {proj.title}
                                 </h3>
                              ...
                              {/* Heynesh Circle Arrow Button */}
                              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/15 backdrop-blur-md border border-white/25 text-white flex items-center justify-center shrink-0 group-hover:bg-orange-500 group-hover:text-black group-hover:border-orange-500 group-hover:scale-110 transition-all duration-300 shadow-xl">
           ...
           <AnimatePresence>
           {openProject && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-2xl overflow-y-auto p-3 md:p-8 flex flex-col">
                 
                 <button onClick={() => setOpenProject(null)} className="fixed top-4 right-4 md:top-8 md:right-8 w-10 h-10 md:w-12 md:h-12 bg-white/20 backdrop-blur-md border border-white/40 text-white rounded-full font-bold text-lg hover:bg-orange-500 transition-colors z-[70] shadow-xl flex items-center justify-center">✕</button>
                 
                 <div className="max-w-4xl mx-auto w-full pt-16 md:pt-20 pb-16 relative">
                    <div className="absolute top-1/4 left-0 w-[300px] h-[300px] bg-orange-400/20 blur-[90px] rounded-full pointer-events-none"></div>

                    <h2 className="text-2xl md:text-4xl font-black text-white mb-6 md:mb-8 text-center px-4 relative z-10 drop-shadow-lg">{openProject.title}</h2>
                    
                    {openProject.image && (
                        <div className="mb-6 md:mb-8 relative z-10">
                            <DarazZoomImage src={urlFor(openProject.image).url()} alt={openProject.title} />
                            <p className="text-center text-[10px] md:text-xs text-gray-300 mt-2 uppercase tracking-widest font-bold">Hover to zoom</p>
                        </div>
                    )}
