...
// ইমেজের সাইজ এবং প্যাডিং মোবাইলের জন্য অপ্টিমাইজ করা হয়েছে
...
// ফন্ট সাইজগুলো মোবাইল (text-sm, text-xl) এবং ডেস্কটপের (md:text-lg, md:text-3xl) জন্য আলাদা করে রেসপন্সিভ করা হয়েছে
const ptComponents = {
  types: {
    image: ({ value }: any) => {
...
export default function Portfolio({ projects, openProject, setOpenProject, isHomePage = true }: any) {
    ...
    return (
        <div className="w-full">
           <h2 className="text-3xl md:text-5xl font-black text-center mb-8 md:mb-12 text-black">Projects</h2>

           {/* এখানেই আসল ম্যাজিক! মোবাইলে ২ কলাম (columns-2) এবং ডেস্কটপে ৩ কলাম (md:columns-3) করা হয়েছে */}
               ...
               {displayProjects.map((proj: any) => (
                  ...
                  >
                     ...
                     <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3 md:p-5">
                        <h3 className="text-white text-sm md:text-xl font-bold tracking-wide leading-snug line-clamp-2 md:line-clamp-none">{proj.title}</h3>
                        <p className="text-orange-400 font-bold mt-1 flex items-center gap-1 md:gap-2 text-[10px] md:text-sm">View Project <span className="text-sm md:text-lg">↗</span></p>
                     </div>
                  </motion.div>
               ))}
           </div>
           ...
           {isHomePage && projects.length > 6 && (
             <div className="flex justify-center mt-8 md:mt-12">
                <Link href="/project" className="px-6 py-3 md:px-8 bg-black text-white text-sm md:text-base font-bold rounded-full hover:bg-gradient-to-r hover:from-orange-500 hover:to-orange-600 hover:shadow-[0_10px_20px_rgba(249,115,22,0.3)] transition-all duration-300 hover:-translate-y-1">
           ...
           {openProject && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9990] bg-[#F9F9F6]/95 backdrop-blur-xl overflow-y-auto p-3 md:p-8 flex flex-col">
                 ...
                 <div className="max-w-4xl mx-auto w-full pt-16 md:pt-24 pb-16">
                    ...
                    <div className="bg-white p-5 md:p-10 rounded-2xl md:rounded-[2rem] shadow-lg border border-gray-100">
                        {openProject.description ? (
                            <PortableText value={openProject.description} components={ptComponents} />
                        ) : (
                            <p className="text-gray-500 text-center italic text-sm md:text-base">No description available for this project.</p>
                        )}
                    </div>
                 </div>
              </motion.div>
